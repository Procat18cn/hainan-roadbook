...
| 🎯 地图点选反查 | 「📍 添加地点」点在地图上任意位置，自动列出附近的高德地点（名称/地址/距离），点选即带名加入行程或修正现有停留点；无 Key / 无结果 / 断网自动退回直接落点 |
...
不填 Key 也能使用全部其他功能（手绘路线用估算距离；「📍 添加地点」退回直接落点）；填了 Key 才能用 **地图搜索**、**🎯 点选反查** 和 **⚡ 高德算路**。注册步骤约 5 分钟：
...
**断网了还能用吗？**
底图瓦片需要联网；断网时海岛轮廓（内嵌边界）、路线、站点、备注、海报生成均正常可用，仅高德 POI 搜索、点选反查与算路不可用。
