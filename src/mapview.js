...
MV.renderAll = function () {
  ...
  p.days.forEach((d, di) => {
    ...
    d.stops.forEach((s, i) => {
      ...
      // 弹窗创建时一次性绑定，开/关交给 Leaflet 内建 toggle。
      // 不可点击时再 bindPopup+openPopup：1.1.1 的 bindPopup 会自动注册 click toggle 处理器，
      // 与自定义 click 叠加后，第二次点击会被其“已打开则关闭”分支立即误关
      mk.bindPopup(MV.stopPopupHtml(d, di, s, myNum), { maxWidth: 320 });
      mk.addTo(group);
    });
  });
};
...
/* 空关键词在 JS API 2.0 实测只覆盖购物等商业类 POI，显式声明大类才能查到景点/餐饮/住宿等 */
const PICK_TYPES = '风景名胜|餐饮服务|住宿服务|购物服务|生活服务|交通设施服务|科教文化服务|地名地址信息';

Pick.start = function (latlng) {
  if (!App.settings.amapKey) { Panel.addStopAt(latlng); return; } // 无 Key：保持原直接落点行为
  toast('查询附近地点…');
  ensureAMap().then(() => {
    AMap.plugin('AMap.PlaceSearch', () => Pick._query(latlng, pickRadius(), true));
  }).catch((e) => {
    toast('附近地点查询失败：' + (e && e.message ? e.message : '网络异常') + '，已直接落点', 'err');
    Panel.addStopAt(latlng);
  });
};

/* 查询一次；零结果且为首查时半径翻倍重试一次，仍空则明示原因后退回直接落点 */
Pick._query = function (latlng, radius, first) {
  const ps = new AMap.PlaceSearch({ city: '海南省', citylimit: true, pageSize: 8, extensions: 'base', type: PICK_TYPES });
  // 底图为高德瓦片时 Leaflet 点击坐标即 GCJ 显示坐标，可直接交给高德接口
  ps.searchNearBy('', [latlng.lng, latlng.lat], radius, (status, result) => {
    ...
    if (usable.length) { Pick.showPopup(latlng, usable); return; }
    if (first) { Pick._query(latlng, Math.min(radius * 2, 1000), false); return; }
    if (status !== 'complete' && result && result.info && result.info !== 'OK') {
      toast('附近查询失败（' + String(result.info) + '），已直接落点', 'err');
    } else {
      toast('附近未找到收录地点，已直接落点，可手动命名');
    }
    Panel.addStopAt(latlng);
  });
};
